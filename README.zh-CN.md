...
> **文档状态：**维护中的指南和架构参考已于 2026-09-10 根据源码树、OpenAPI 契约、Compose 拓扑和 Alembic 迁移链完成核对。带日期的审计报告仍作为历史证据保留；请从 [docs/README.md](docs/README.md) 进入当前文档导航。

## 界面预览

由 31 张截图制作的动态导览，包含所提供的全部聊天界面截图及长回答的后续画面，展示素材管理、PDF/幻灯片与视频预览、限定范围的带引用问答、历史记录、Memory、设置，以及基于 Skill 的文档生成。每个画面均留有阅读时间；播放节奏不代表应用的实际响应速度。
...
- **层次化混合检索** —— 先由会议/文件摘要 Router 与 Funnel 缩小范围，再融合 Chroma 与 BM25/FTS5 候选，经过 RRF、按文件公平分配、会话锚点及可选 Cohere/BGE 重排。
- **保守查询路由** —— 按答案形态把请求划分为原子事实、有限综合和分析综合；检索后的 Evidence Filter 会把证据弱或跨来源冲突的原子探测提升回完整流程。
...
- 前端：[http://localhost:8307](http://localhost:8307)
- 后端 API：[http://localhost:7008](http://localhost:7008)
- API 文档：[http://localhost:7008/docs](http://localhost:7008/docs)
...
| `LLM_API_KEY`               | _必需_                   | 所选 LLM 提供商的 API Key                                                                                                                                   |
| `LLM_BASE_URL` / `LLM_HOST` | _空_                     | OpenAI 兼容或本地提供商的自定义端点                                                                                                                         |
...
| `ASSEMBLYAI_API_KEY`        | _处理音视频时必需_       | 只能通过环境变量设置，不能写入 YAML                                                                                                                         |
...
| `API_KEY`                   | _空_                     | 空值表示开发模式（绕过鉴权）；staging/production 必须设置                                                                                                   |
| `PRINCIPAL_PEPPER`          | _空_                     | 非开发环境中生成稳定、不可逆主体标识所需的密钥                                                                                                              |
| `PRINCIPAL_ID`              | _未设置_                 | 可选的、已验证的现有所有者 ID，用于 API Key 轮换时保持归属；它不是多用户身份认证                                                                            |
...
所有产品 API 路由都使用 `/api/v1` 版本前缀；Prometheus `/metrics` 端点有意位于该前缀之外。受保护路由通过 `X-API-Key` 请求头认证（`API_KEY` 为空表示开发模式）；存活、就绪和基础健康探针有意保持无需认证。限流按端点设置：上传/聊天 20 次/分钟、设置 5 次/分钟、读取 60 次/分钟。错误统一使用 `ErrorResponse` 包装（`code`、`message`、`request_id`、`details`）。完整的生成契约导航请参见 [backend/docs/api-reference.md](backend/docs/api-reference.md)；下表是面向产品的精简索引。
...
| `GET`    | `/meetings/{id}/summary`                           | 获取摘要及其生命周期状态                      |
...
| `GET`  | `/chat/run-lookup`                        | 根据幂等客户端/运行键查找持久化运行                                                                                 |
| `GET`  | `/chat/runs/{run_id}` / `events`          | 查看持久化运行记录并重放事件                                                                                        |
| `POST` | `/chat/run-cancel`                        | 根据幂等运行键取消生成                                                                                              |
...
| `GET`                             | `/memory/projects/materials`    | 列出可分配给项目的会议素材          |
...
| `POST`                            | `/memory/retry-index`           | 重试等待中或失败的记忆向量写入      |
...
| `GET` / `POST` | `/settings/account/deletions/{batch_id}` / `retry` | 查看或重试外部账户清理任务                                         |
| `GET`          | `/health`                                          | 完整依赖健康检查                                                   |
...
| `POST`         | `/health/reset-memory-cb`                          | 重置 Memory 断路器                                                 |
| `POST`         | `/ws/token`                                        | 签发短期 WebSocket 令牌                                            |
| `WS`           | `/ws`                                              | 实时进度/完成通知                                                  |
...
下表是唯一的当前得分摘要，生成相关套件的最新验证完成于 **2026-09-09 17:39 UTC**。机器可读结果、报告哈希、模型角色与限制记录在 [docs/validation/latest-benchmark.json](docs/validation/latest-benchmark.json) 中。

Chat、RAG Answer、Multi-turn 和 E2E 已在移除错误的 2.5 秒硬生成截断后重新运行；不受影响的套件保留各自最近一次结果。因此，下表是“各套件最新结果”的汇总，而不是同一时刻完成的一次原子化全套运行。
...
| RAG 回答                 | 10 个用例 × 3 次评审重复 | 忠实度 **0.883**；相关性 **0.995**；上下文精确率 **0.967**；上下文召回率 **1.000**；正确性 **1.000**；引用质量 **0.927** | 有效的合成诊断；两次裁判请求在提供商 504 后重试成功  |
| 多轮对话                 | 6 个用例 × 1 次评审重复  | 忠实度 **0.996**；合适性 **1.000**；自然度 **0.983**；完整性/证据召回/会话连续性均为 **1.000**                           | 仅为诊断；评审只重复一次                             |
...
| 上传 → 就绪 → 带引用聊天 | 1 个隔离 fixture         | 就绪 **4.09 秒**；TTFT **15.05 秒**；总耗时 **15.37 秒**；引用/事实/来源检查通过                                         | 功能检查通过；**3 秒 SLO 门控失败**                  |
| 主模型 Chat              | 20 次请求                | 20/20 完成；降级率 **0%**；TTFT P95 **22.93 秒**；总耗时 P95 **25.17 秒**                                                | **SLO 门控失败**                                     |

验证时的模型角色分别是：`z-ai/glm-5.3-flash` 用于主生成，`qwen/qwen3.8-flash` 用于独立评审，`deepseek/deepseek-v4-flash-vision-exp` 用于 Memory 抽取和视觉任务。这些是面向本项目合成数据的诊断，不是仓库默认配置，也不是通用模型排名。OpenRouter 提供商端点没有固定到特定后端。由于两个 Chat 性能门控均未通过、公开质量集规模较小且为合成数据、多轮对话仅进行一次评审、视觉仅使用一张截图，并且运行中观察到提供商 504 及查询改写/指代消解超时，系统目前仍然**未达到发布就绪状态**。

...
async def _run_single_combination(
    ...
    with bench_environment():
        ...
        fetch_multiplier = (
            settings.RAG_RERANK_FETCH_MULTIPLIER if reranker_binding else 1
        )
...
def _aggregate(rows: list[dict]) -> dict[str, float]:
    if not rows:
        return {}
    keys = [k for k in rows[0].keys() if k != "query_id"]
    ...
    for cfg_data in top_2_configs:
        ...
        for provider, reranker in RETRIEVAL_GRID:
            ...
            run_result = asyncio.run(
                _run_single_combination(cfg, provider, reranker, scoped_items, unscoped_items, top_k)
            )

...
def _build_scoped_prompt(chunks_text: str, n: int) -> str:
    return (
        ...
        f"3. For each question, you MUST specify which meeting_id and file_id the answer comes from.\n"
...
def _build_unscoped_prompt(summaries_text: str, n: int) -> str:
    return (
        ...
        f"3. For each question, you MUST list all meeting_id and file_id values involved in the answer.\n"
        f"   The summaries above are prefixed with [meeting_id=X, file_id=Y]. Use these exact IDs.\n"
        f"4. Answer length must be mixed: about half should be SHORT (1-3 sentences), "
        f"   and about half should be DESCRIPTIVE (a full paragraph with details).\n"
        f"5. Output as JSON array with this exact schema:\n"
        f'   [{{'
...
async def generate_queries_from_chunks(
    ...
    for item in items:
        ...
        if isinstance(mids, str):
            item["expected_meeting_ids"] = [int(x.strip()) for x in mids.split(",") if x.strip().isdigit()]
        else:
            item["expected_meeting_ids"] = [int(x) for x in mids if isinstance(x, (int, str)) and str(x).isdigit()]
        if isinstance(fids, str):
            item["expected_file_ids"] = [int(x.strip()) for x in fids.split(",") if x.strip().isdigit()]
        else:
            item["expected_file_ids"] = [int(x) for x in fids if isinstance(x, (int, str)) and str(x).isdigit()]
...
async def main() -> None:
    ...
    parser.add_argument("--output-scoped", type=Path, default=FIXTURE_DIR / "amicorpus_golden_scoped.json")
    parser.add_argument("--output-unscoped", type=Path, default=FIXTURE_DIR / "amicorpus_golden_unscoped.json")
